...
- **Optional viewer parallax** — off by default; strength now ranges from Minimal, capped to a few pixels, through Extreme, which boosts distant perspective anchors. The default parallax type is raised card lift, with perspective anchors selectable from scene center/corners, per-Region center/corners, a draggable point on the scene edge in Elevation select mode, or the camera-relative furthest scene edge. Region center restores the older camera-relative local direction, with additional damped, camera-origin, and velocity parallax types for reducing center-crossing snap.

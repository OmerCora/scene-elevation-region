...
- **Per-scene elevation settings** — the control-group settings button opens a scene palette for parallax, transition, perspective, shadow, overlay scale, token elevation policy, and token scaling overrides.
- **Elevation Region behavior** — `Elevation`, `Shadow strength`, per-Region parallax/shadow/transition overrides, `Modify token elevation`, and `Modify token scaling`.

...
- **Elevation Region behavior** — `Elevation`, `Shadow strength`, per-Region preset, parallax strength/type, perspective point, overlay scale, shadow type/length, transition, and depth scale overrides, `Modify token elevation`, and `Modify token scaling`.
